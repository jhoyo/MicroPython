...
# Pines de salida
luz1 = Pin(19, Pin.OUT) # Interruptor de Luz
comp = Pin(18, Pin.OUT) # Compresor A/C
alar = Pin(5, Pin.OUT)  # Alarma iluminacion
    ...
    while True:
        try:
            ...
            if (time() - t0) >= tf1: # enviar temperatura y humedad cada 30 segundos
                env_temp1(round(bme.temperature, 2))
                env_hum(round(bme.humidity, 2)) 
                ds1.convert_temp()
                sleep_ms(750)
                temp2 = 1
            ...
            if (time() - t1) >= tf2: # enviar presion e iluminacion cada 15 segundos
                env_pres(round(bme.pressure,2))
                env_ilum(round(ltr.ALS(),2))
                t1 = time()
            sleep(1)     
